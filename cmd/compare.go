...
func newAPICompare(ctx context.Context,
	...
	return &apiCompare{
		...
		lAPI:    lAPI,
		dp:      dp,
		handler: newHandler(ctx, vAPI, lAPI),
	}
}
...
func (ac *apiCompare) CompareStateGetRandomnessFromTickets() error {
	per := crypto.DomainSeparationTag_TicketProduction
	randEpoch := ac.dp.currentTS.Height()
	entropy := []byte("fixed-randomness")
	key := ac.dp.currentTS.Key()
	...
	for per < crypto.DomainSeparationTag_PoStChainCommit {
		req := newReq(stateGetRandomnessFromTickets, toInterface(ac.ctx, per, randEpoch, entropy, key))
		ac.handler.send(req)
		if err := <-req.err; err != nil {
			return err
		}
...
func (ac *apiCompare) CompareStateGetRandomnessDigestFromTickets() error {
	randEpoch := ac.dp.currentTS.Height()
	key := ac.dp.currentTS.Key()

	return ac.sendAndWait(stateGetRandomnessDigestFromTickets, toInterface(ac.ctx, randEpoch, key))
}

func (ac *apiCompare) CompareStateGetRandomnessDigestFromBeacon() error {
	randEpoch := ac.dp.currentTS.Height()
	key := ac.dp.currentTS.Key()
...
func (ac *apiCompare) CompareEthGetTransactionCount() error {
	...
	h := types.EthUint64(ac.dp.currentTS.Height())
	bh := types.EthBlockNumberOrHash{
		BlockNumber: &h,
	}

	return ac.sendAndWait(ethGetTransactionCount, toInterface(ac.ctx, sender, bh))
}
...
func (ac *apiCompare) CompareEthGetTransactionByBlockHashAndIndex() error {
	return ac.sendAndWait(ethGetTransactionByBlockHashAndIndex, toInterface(ac.ctx, emptyEthHash, types.EthUint64(0)), withExpectCallAPIError())
}

func (ac *apiCompare) CompareEthGetTransactionByBlockNumberAndIndex() error {
	height := ac.dp.currentTS.Height()
	return ac.sendAndWait(ethGetTransactionByBlockNumberAndIndex, toInterface(ac.ctx, types.EthUint64(height), types.EthUint64(0)), withExpectCallAPIError())
}
...
func (ac *apiCompare) CompareEthGetCode() error {
	...
	bh := types.EthBlockNumberOrHash{
		PredefinedBlock: &blkParams[1],
	}

	return ac.sendAndWait(ethGetCode, toInterface(ac.ctx, addr, bh))
}
...
func (ac *apiCompare) CompareEthGetStorageAt() error {
	...
	return ac.sendAndWait(ethGetStorageAt, toInterface(ac.ctx, addr, types.EthBytes{}, bh))
}
...
func (ac *apiCompare) CompareEthGetBalance() error {
	...
	return ac.sendAndWait(ethGetBalance, toInterface(ac.ctx, addr, bh))
}
...
// func (ac *apiCompare) CompareEthTraceBlock() error {
// 	for _, param := range blkParams {
// 		if err := ac.sendAndWait(ethTraceBlock, toInterface(ac.ctx, param)); err != nil {
// 			return fmt.Errorf("block param: %s, error: %v", param, err)
// 		}
// 	}
...
// func (ac *apiCompare) CompareEthTraceReplayBlockTransactions() error {
// 	for _, param := range blkParams {
// 		if err := ac.sendAndWait(ethTraceReplayBlockTransactions, toInterface(ac.ctx, param, []string{"trace"})); err != nil {
// 			return fmt.Errorf("block param: %s, error: %v", param, err)
// 		}
// 	}
